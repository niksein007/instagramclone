...
class MainCard extends React.Component {
   ...
   state={
    ...
    render(){
        ...
        const slides = this.props.uploadPic.map((index)=>{
            return <span key={index} index ={index}className={index===this.state.count? "active": "slide"}></span>
        })
	...
	return (
        ...
        {/*  creating a slide show or carousel */}
			<div className="mainPost" >
                <p className="count"> <span>{this.state.count + 1} </span>/{uploadPics.length}</p>
				<div className="mainImg">{uploadPics[this.state.count]}</div>

                <span className ={this.state.count === 0?"toggle":"prev"} onClick={this.prev} > &#10094;</span>
                <span className={ this.state.count === uploadPics.length - 1 ?"toggle":"next"} onClick={this.next}>&#10095;</span>
			</div>
